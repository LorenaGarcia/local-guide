        ...
        <div className="flex-1 p-8 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-2xl font-black text-[#1F4D47] leading-tight group-hover:text-[#2D9C8D] transition-colors">
                {name}
              </h3>
            </div>
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {tags &&
                tags.map((tag, index) => (
                  <span
                    key={index}
                    className="px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] border border-slate-100 text-slate-400"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <div className="flex justify-between items-center pt-6 border-t border-slate-50">
            <div className="flex items-baseline gap-1.5 pr-4">
              {address && (
                <span className="text-slate-400 font-bold text-xs tracking-wider line-clamp-2">
                  {address}
                </span>
              )}
            </div>
