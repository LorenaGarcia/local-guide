...
function getActiveSocialMedia(socialLinks: SocialMediaLinks | null | undefined) {
    const activeSocialMedia = socialLinks
        ? COMPANYS_SOCIAL_MEDIA.map(social => {
            ...
            if (social.label === 'WhatsApp') url = socialLinks.whatsapp ? `https://wa.me/${socialLinks.whatsapp}` : "";
            return { ...social, url };
        }).filter(s => s.url)
        : [];
...
function getBusinessDetailData(blok: BusinessDetailBlok) {
    ...
    const gallery = Array.isArray(rawGallery)
        ? rawGallery.map((img: StoryblokImage | string) => typeof img === 'string' ? img : img.filename).filter(Boolean)
        : [];
    const image = (blok.image && typeof blok.image === 'object' ? blok.image.filename : blok.image) || gallery[0] || "";
    ...
    const socialLinks = Array.isArray(blok.socila_links) && blok.socila_links.length > 0 ? blok.socila_links[0] : null;
