...
import { BUSINESSES, COMPANYS_SOCIAL_MEDIA } from '@/constants';
import Link from 'next/link';
import { notFound } from 'next/navigation';
import { GalleryModal } from '@/components/gallery-modal';

function BusinessDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const {image, gallery, name, category, rating, reviewsCount, location, isFeatured, fullDescription, description, amenities, workshops, hours} = biz;
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-10 bg-white min-h-screen">
      <div className="flex items-center gap-3 text-[13px] font-bold text-slate-400 mb-8 overflow-hidden whitespace-nowrap">
        ...
        <span className="hover:text-[#2D9C8D] transition-colors shrink-0 cursor-pointer">{category}</span>
        <span className="text-slate-300 font-normal">›</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-12">
        ...
        >
          <img src={gallery?.[0] || image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" alt={name} />
          <div className="absolute top-6 left-6 bg-white/95 backdrop-blur-md px-4 py-2 rounded-xl flex items-center gap-2 shadow-sm border border-white/20">
            ...
            <span className="text-[#1F4D47] font-bold text-[13px]">{rating} ({reviewsCount} reseñas)</span>
          </div>
        </div>
        <div className="hidden lg:flex lg:col-span-4 flex-col gap-5">
          ...
          >
            <img src={gallery?.[1] || image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" alt="Gallery 1" />
          </div>
          <div 
            className="flex-1 relative rounded-[2rem] overflow-hidden group cursor-pointer h-1/2"
          >
            <img src={gallery?.[2] || image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" alt="Gallery 2" />
            {!!gallery?.length && (
              <div onClick={() => openGallery(gallery?.length || 0)} className="absolute inset-0 bg-black/40 flex items-center justify-center group-hover:bg-black/50 transition-colors">
                <div className="text-white flex items-center gap-2">
                  <span className="material-symbols-outlined text-xl">grid_view</span>
                  <p className="font-bold text-sm tracking-tight">Ver las {gallery?.length || 0} fotos</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
        <div className="lg:col-span-8">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <span className="bg-[#F1F5F9] text-slate-500 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-wider">{category || 'Fitness y Bienestar'}</span>
                {isFeatured && (
              ...
              <h1 className="text-5xl lg:text-6xl font-black text-[#1F4D47] mb-4 tracking-tight">{name}</h1>
          ...
          <div className="flex flex-wrap gap-3 mb-16 pb-12 border-b border-slate-50">
            {COMPANYS_SOCIAL_MEDIA.map((social, i) => (
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border border-slate-50 shadow-sm">
              ...
              <div className="space-y-5 mb-10">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-[#1F4D47] font-black">{hours?.weekdays}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-2.5 text-slate-400 font-bold">
                    <span className="material-symbols-outlined text-[18px]">schedule</span> Sáb - Dom
                  </div>
                  <span className="text-[#1F4D47] font-black">{hours?.weekends}</span>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-50">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="text-sm">
                    <p className="text-slate-600 font-medium leading-relaxed">{location?.address}</p>
