...
import { CATEGORIES_BUSINESS } from '@/constants';
import { Business } from '@/components/business';
import { CategoryFilter } from '@/components/category-filter';
import { HighlightedBusiness } from '@/types';

interface BusinessPageClientProps {
  initialBusinesses: HighlightedBusiness[];
}

function BusinessPageContent({ initialBusinesses }: BusinessPageClientProps) {
  ...
  const filteredBusinesses = initialBusinesses.filter(biz => {
    ...
    return biz.tags && biz.tags.some(tag => tag.toUpperCase() === selectedCategory.toUpperCase());
  });
...
export default function BusinessPageClient({ initialBusinesses }: BusinessPageClientProps) {
  return (
    <Suspense fallback={
      ...
      <BusinessPageContent initialBusinesses={initialBusinesses} />
    </Suspense>
  );
}
